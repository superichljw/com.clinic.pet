...
<body>
	<%@ include file='./instapageheader.jsp'%>
	...
	<div class="mypageBack">
		...
		<div class="instaRight">
			<h1>메인페이지</h1>
			<p>로그인한 아이디 : ${loginUser.userid }</p>
			<p>테스트용 : ${notfollow.following }</p>
			<input type="button" value="내 페이지로 돌아가기"
				onclick="location.href='../main'">
			<c:set var="loginid" value="${loginUser.userid }"></c:set>
			<c:set var="notfollow" value="${notfollow.following }" />
			<h2>${instavo.userid }</h2>
			<c:choose>
				<c:when test="${empty instavo.profileimage }">
					<img style="width: 150px; height: 150px;"
						src="/pet/image/upload/defaultface.png">
				</c:when>
				<c:otherwise>
					<img style="width: 150px; height: 150px;"
				...
				<c:otherwise>
					<table>
						<c:forEach items="${myimage }" var="myimage" varStatus="status">
							...
							<td><a href="imagedetailpage?imageno=${myimage.imageno }"><img

...
<body>
	...
	<div id="tab">
		...
		<div class="tabmenu">
			...
			<button class="btn3 on" type="button" style="margin-left: -5px;"
		...
		<div class="tabcontent">
			<c:forEach items="${faqdtos }" var="faqdtos">
				<ul>
					<li style="width: 10%; border-right: 1px solid lightgray;"><a
						href="./faq_content_view?faq_num=${faqdtos.faq_num }">${faqdtos.faq_num }</a></li>
					<li
						style="width: 49.5%; margin-left: -5px; border-right: 1px solid lightgray;">
						<a href="./faq_content_view?faq_num=${faqdtos.faq_num }">${faqdtos.faq_title }</a>
					</li>
					<li
						style="width: 15%; margin-left: -5px; border-right: 1px solid lightgray;"><fmt:formatDate
							value="${faqdtos.faq_date }" /></li>
					<li
						style="width: 14.5%; margin-left: -5px; border-right: 1px solid lightgray;">${faqdtos.faq_name }</li>
					<li style="width: 10%; margin-left: -5px;">${faqdtos.faq_hit }</li>
				</ul>
			</c:forEach>
		</div>
		...
		<div class="board-pageNum">
				<tr>
					<td colspan="5"><a
						href="./faq_list?page=${faqPage.firstPageNum }
			&pageDataCount=${faqPage.pageDataCount}">
							◀◀맨앞으로 </a>&nbsp;&nbsp; <a
						href="./faq_list?page=${faqPage.prevPageNum }
			&pageDataCount=${faqPage.pageDataCount}">
							◀앞으로 </a>&nbsp;&nbsp; <c:forEach var="i"
							begin="${faqPage.startPageNum }" end="${faqPage.endPageNum }"
							step="1">
							<c:choose>
								<c:when test="${i eq faqPage.currentPageNum }">
									<a style="font-weight: bold; color: blue"
										href="./faq_list?page=${i }
				&pageDataCount=${faqPage.pageDataCount}">${i }
									</a>&nbsp;&nbsp;
			</c:when>
								<c:otherwise>
									<a
			...
			&pageDataCount=${faqPage.pageDataCount}">${i }
						...
						href="./faq_list?page=${faqPage.nextPageNum }
			&pageDataCount=${faqPage.pageDataCount}">뒤로▶
					</a>&nbsp;&nbsp; <a
						href="./faq_list?page=${faqPage.lastPageNum }
			&pageDataCount=${faqPage.pageDataCount}">맨뒤로▶▶
					</a></td>
				</tr>
		</div>
	</div>
	...
	<c:choose>
		<c:when test="${empty loginUser }">
			<div
				...
				<a href="./faq_write_form"><input type="button" value="글쓰기"
					id="writeBtn"></a>
			</div>
		</c:when>

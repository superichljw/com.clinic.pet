...
<body>
	...
	<div>
		...
		<form name="frm" method="post" action="./qna_write" enctype="multipart/form-data">
			<table class="boardWriteTable">
				<c:choose>
					<c:when test="${loginUser.usertype eq 'oper' }">
						<tr>
							<th>작성자</th>
							<td><input type="text" name="qna_name" class="table-name"
								value="관리자" size="60" readonly> * 필수</td>
						</tr>
					</c:when>
					<c:otherwise>
						<tr>
							<th>작성자</th>
							<td><input type="text" name="qna_name" class="table-name"
								value="${loginUser.userid }" size="60"> * 필수</td>
						</tr>
					</c:otherwise>
				</c:choose>
				<tr>
					<th>비밀번호</th>
					<td><input type="password" size="101" name="qna_password"
						class="table-title" style="font-family:gulim"> * 필수</td>
				</tr>

				<tr>
					<th>제목</th>
					<td><input type="text" size="101" name="qna_title"
						class="table-title"> * 필수</td>
				</tr>
				<tr>
					<th>내용</th>
					<td><textarea cols="100" rows="20" name="qna_content"
							class="table-content" id="content"></textarea></td>
				</tr>
			</table>
			<br> <br>
			<div class="boardButtons">
				<input type="submit" value="등 록" class="boardBtnApply"
					onclick="updateSe()"
				...
				<input type="button" value="목 록" class="boardBtnList"
					style="border: none; font-size: 20px; padding: 10px 20px; margin-left: 10px;"
					onclick="location.href='/web-project/faqpage.do'">
			</div>

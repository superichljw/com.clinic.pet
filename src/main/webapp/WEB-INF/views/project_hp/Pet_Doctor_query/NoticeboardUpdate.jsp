...
<body>
	...
	<div>
		<h1 style="text-align: center; padding: 20px;">게시글 수정하기</h1>
		<form name="frm" action="./board_update" method="post" enctype="multipart/form-data">
		<input type="hidden" name="num" value="${board.num }">
			<table class="boardWriteTable">
				<tr>
					<th>작성자</th>
					<td><input type="text" name="name" class="table-name"
						size="60" value="${board.name }"> * 필수</td>
				</tr>
				<tr>
					<th>비밀번호</th>
				...
				<tr>
					...
					<td><input type="text" name="email" class="table-email"
						value="${board.email }" size="60"></td>
				</tr>
				<tr>
					<th>제목</th>
					<td><input type="text" size="101" name="title"
						value="${board.title }" class="table-title"> * 필수</td>
				</tr>
				<tr>
					<th>내용</th>
					<td><textarea cols="100" rows="20" name="content" id="content"> ${board.content }</textarea></td>
				</tr>
			</table>
			<br> <br>
			<div class="boardButtons">
				<input type="submit" value="등 록" onclick="boardCheck();updateSe();"
					class="boardBtnApply"
